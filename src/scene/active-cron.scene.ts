...
@Scene('ACTIVE_CRON_SCENE')
export class ActiveCronScene {
  private readonly logger = new Logger('ActiveCronSceneLogger');
  ...
  async onSceneEnter(@Ctx() ctx: SceneContext) {
    ...
    if (notifications.length === 0) {
      await ctx.reply('Tus notificationes estan todas activas.');
      await ctx.scene.leave();
    ...
    await ctx.reply(
      `¿Que notificación querés activar?\n\n${notificationsString}`,
  ...
  async onSelectNotification(@Ctx() ctx: SceneContext) {
    ...
    await ctx.reply('La notificación fue activada.');
    await ctx.scene.leave();
  }
